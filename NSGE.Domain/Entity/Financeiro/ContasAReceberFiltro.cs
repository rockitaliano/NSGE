...
{
    public class ContasAReceberFiltro
    {
        public ContasAReceberFiltro()
        {
            this.PeriodoTipo = "V";
        ...
        public KeyValuePair<string, Dictionary<string, object>> MontarWhereSql()
        {
            ...
            {
                where.Add("NomedoClienteRazaoSocial like @cliente");
                parameters.Add("@cliente", string.Format("%{0}%", this.Cliente));
            }
            ...
            if (!string.IsNullOrEmpty(this.IdDoCentroDeCusto))
            {
                ...
                parameters.Add("@IdDoCentroDeCusto", IdDoCentroDeCusto);
            }
            ...
            if (this.StatusPagamento.HasValue)
            {
                ...
                {
                    where.Add("(StatusRecebimento = @aberto OR StatusRecebimento = @vencido)");
                ...
                {
                    where.Add("StatusRecebimento = @StatusRecebimento");
                    parameters.Add("@StatusRecebimento", StatusPagamento.Value.GetEnumValue());
                }
            }
        ...
        public string MontarDescricaoDosFiltrosSelecionados()
        {
            ...
            filtros.Append(string.IsNullOrEmpty(Cliente) ? "todos os clientes, " : string.Format("só cliente {0}, ", Cliente));
